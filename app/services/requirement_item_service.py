...
from app.api.schemas.requirements import RequirementItemCreate, RequirementItemUpdate, RequirementItemUpsert, RequirementItemBatchUpsert
...
class RequirementItemService:
    ...
    def __init__(
        ...
        Args:
            repository: The requirement item repository.
            project_repository: The project repository for validation.
        """
        self.repository = repository
        self.project_repository = project_repository
    
    def create_requirement_item(self, db: Session, item_data: RequirementItemCreate) -> RequirementItem:
        ...
        """
        logger.info(f"Creating requirement item for project {item_data.project_id}: {item_data.title}")
        
        try:
            # Validate that the project exists
            project = self.project_repository.get(db, item_data.project_id)
            if not project:
                logger.warning(f"Attempted to create requirement item for non-existent project: {item_data.project_id}")
            ...
            logger.info(f"Successfully created requirement item {created_item.id} for project {item_data.project_id}")
            
            # Log audit trail
            logger.info(f"AUDIT: RequirementItem created - ID: {created_item.id}, Project: {item_data.project_id}, "
            ...
            logger.error(f"Failed to create requirement item for project {item_data.project_id}: {str(e)}", exc_info=True)
            raise DatabaseException(
                f"Error creating requirement item: {str(e)}", 
    ...
    def list_requirement_items(
        ...
        try:
            ...
            if project_id:
                ...
                return self.repository.get_by_project(db, project_id, status, skip, limit)
            else:
            ...
            raise DatabaseException(
                f"Error listing requirement items: {str(e)}", 
                original_exception=e
            )
    ...
    def upsert_requirement_item(
        ...
        Args:
            item_id: The ID of the requirement item (None for create).
        ...
        Raises:
            ...
            DatabaseException: If there's a database error.
        """
        logger.info(f"Upserting requirement item {item_id} for project {item_data.project_id}")
        
        try:
            # Validate that the project exists
            project = self.project_repository.get(self.repository.db, item_data.project_id)
            if not project:
                logger.warning(f"Attempted to upsert requirement item for non-existent project: {item_data.project_id}")
                raise NotFoundException(
                    f"Project with id {item_data.project_id} not found",
                    resource_type="Project",
                    resource_id=item_data.project_id
            ...
            if item_id is None:
                ...
                created_item = self.repository.create(self.repository.db, obj_in=create_data)
                
                # Update status if provided
                if item_data.status is not None:
                    created_item = self.repository.update_status(created_item.id, item_data.status)
                
                logger.info(f"AUDIT: RequirementItem created via upsert - ID: {created_item.id}, "
                           f"Project: {item_data.project_id}, Title: {item_data.title}")
                ...
                if existing_item.project_id != item_data.project_id:
                    raise BadRequestException(
                        f"Cannot change project_id from {existing_item.project_id} to {item_data.project_id}"
                    )
                ...
                logger.info(f"AUDIT: RequirementItem updated via upsert - ID: {item_id}, "
                           f"Project: {item_data.project_id}, Title: {item_data.title}")
            ...
            logger.error(f"Failed to upsert requirement item {item_id} for project {item_data.project_id}: {str(e)}", exc_info=True)
            raise DatabaseException(
                f"Error upserting requirement item: {str(e)}", 
                original_exception=e
            )
    
    def batch_upsert_requirement_items(
        self, 
        batch_data: RequirementItemBatchUpsert
    ) -> Dict[str, Any]:
            ...
            try:
                ...
                results.append({
                    ...
                    "item": {
                        ...
                        "updated_at": result_item.updated_at.isoformat() if result_item.updated_at else None
                    }
                })
                success_count += 1
                ...
                logger.error(f"Unexpected error upserting item {item_id}: {str(e)}", exc_info=True)
                results.append({
                    "id": item_id,
                    "status": "error",
        ...
        logger.info(f"Batch upsert completed: {success_count} success, {error_count} errors")
    ...
    def update_status(
        ...
        try:
            ...
            logger.info(f"AUDIT: RequirementItem status updated - ID: {item_id}, "
                       f"Old Status: {old_status.value}, New Status: {status.value}, "
                       f"Project: {existing_item.project_id}, Title: {existing_item.title}")
            
            logger.info(f"Successfully updated status for requirement item {item_id} from {old_status.value} to {status.value}")
            
            return updated_item
            
        except (NotFoundException, BadRequestException) as e:
            logger.warning(f"Status update failed for requirement item {item_id}: {str(e)}")
            raise
        except Exception as e:
            logger.error(f"Failed to update status for requirement item {item_id}: {str(e)}", exc_info=True)
            raise DatabaseException(
    ...
    def search_requirement_items(
        ...
        try:
            ...
            raise DatabaseException(
                f"Error searching requirement items: {str(e)}", 
                original_exception=e
            )
    ...
    def bulk_update_status(
        ...
        Raises:
            ...
            DatabaseException: If there's a database error.
        """
        logger.info(f"Bulk updating status for {len(item_ids)} requirement items to {status.value}")
        ...
        try:
            ...
            if len(item_ids) > 100:
                logger.warning(f"Attempted bulk status update with {len(item_ids)} items (max 100)")
            ...
            for existing_item in existing_items:
                logger.info(f"AUDIT: RequirementItem bulk status update - ID: {existing_item.id}, "
                           f"Old Status: {existing_item.status.value}, New Status: {status.value}, "
                           f"Project: {existing_item.project_id}, Title: {existing_item.title}")
            
            logger.info(f"Successfully bulk updated status for {len(updated_items)} requirement items to {status.value}")
            ...
            logger.error(f"Failed to bulk update status for items {item_ids}: {str(e)}", exc_info=True)
            raise DatabaseException(
    ...
    def _validate_status_transition(
        self, 
        current_status: RequirementItemStatus, 
        new_status: RequirementItemStatus
    ) -> None:
        ...
        allowed_transitions = {
            ...
            RequirementItemStatus.accepted: [
                RequirementItemStatus.rejected,
                RequirementItemStatus.new  # Allow reopening
            ],
            RequirementItemStatus.rejected: [
                RequirementItemStatus.new,  # Allow reopening
                RequirementItemStatus.accepted  # Allow direct acceptance
            ]
        }
