...
from app.core.exceptions import NotFoundException, DatabaseException, BadRequestException
...
class SystemRepository(CRUDRepository[System, SystemCreate, SystemUpdate]):
    ...
    def get_by_project(
        ...
        try:
            ...
            logger.info(f"Retrieved {len(systems)} systems for project {project_id} in {execution_time}ms")
            
            return systems
            
        except Exception as e:
            logger.error(f"Error retrieving systems for project {project_id}: {e}")
            raise DatabaseException(f"Error retrieving systems: {str(e)}", original_exception=e)
    ...
    def get_by_project_and_name(
        ...
        try:
            return db.query(System).filter(
                and_(System.project_id == project_id, System.name == name)
            ).first()
            
        except Exception as e:
            logger.error(f"Error retrieving system {name} for project {project_id}: {e}")
            raise DatabaseException(f"Error retrieving system: {str(e)}", original_exception=e)
    ...
    def search_systems(
        ...
        try:
            ...
            if filters.description:
                query = query.filter(System.description.ilike(f"%{filters.description}%"))
            ...
            execution_time = int((time.time() - start_time) * 1000)
            logger.info(f"Search returned {len(systems)} systems for project {project_id} in {execution_time}ms")
            
            return systems
            
        except Exception as e:
            logger.error(f"Error searching systems for project {project_id}: {e}")
            raise DatabaseException(f"Error searching systems: {str(e)}", original_exception=e)
    
    def count_by_project(self, db: Session, project_id: str, filters: Optional[SystemSearchFilters] = None) -> int:
        ...
        """
        try:
            query = db.query(func.count(System.id)).filter(System.project_id == project_id)
    ...
    def validate_dependencies(self, db: Session, project_id: str, dependencies: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            existing_systems = db.query(System.name).filter(System.project_id == project_id).all()
            existing_names = {system.name for system in existing_systems}
            
            # Find invalid dependencies (systems that don't exist)
            invalid_dependencies = [dep for dep in dependencies if dep not in existing_names]
    ...
    def detect_circular_dependencies(
        ...
        try:
            ...
            def has_circular_dependency(current: str, target: str, visited: set, path: List[str]) -> bool:
                ...
                for dep in dependency_graph.get(current, []):
                    if has_circular_dependency(dep, target, visited.copy(), path.copy()):
                        return True
            ...
            logger.error(f"Error detecting circular dependencies for system {system_name}: {e}")
            raise DatabaseException(f"Error detecting circular dependencies: {str(e)}", original_exception=e)
    ...
    def upsert_system(
        ...
        try:
            ...
            existing_system = self.get_by_project_and_name(db, project_id, name)
            ...
            if existing_system:
                ...
                execution_time = int((time.time() - start_time) * 1000)
                logger.info(f"Updated system {name} for project {project_id} in {execution_time}ms")
                ...
                execution_time = int((time.time() - start_time) * 1000)
                logger.info(f"Created system {name} for project {project_id} in {execution_time}ms")
    ...
    def get_systems_by_type(
        ...
        try:
            ...
            logger.error(f"Error retrieving {system_type.value} systems for project {project_id}: {e}")
            raise DatabaseException(f"Error retrieving systems by type: {str(e)}", original_exception=e)
