...
from fastapi import APIRouter, Depends, Request, BackgroundTasks, HTTPException, requests, status
...
from app.api.schemas.llm import LLMStreamSORequest
...
# @router.post("/project_id/{project_id}/stream")
# async def stream_llm_response(
#     project_id: str,
#     request: LLMStreamSORequest,
#     background_tasks: BackgroundTasks,
#     llm_streaming_service: LLMStreamingService = Depends(get_llm_streaming_service),
#     ollama_client: StreamingOllamaClient = Depends(get_ollama_client),
#     so_rag_service: SORAGService  = Depends(get_so_rag_service)
# ) -> EventSourceResponse:
#     """Stream an LLM response to a client using SSE.
...
#     Args:
#         project_id: The project the chat is about
#         client_id: The ID of the client to stream to
#         request: The LLM request
#         background_tasks: FastAPI BackgroundTasks for async processing
#         llm_streaming_service: The LLM streaming service instance
...
#           # 1) Rephrase με RAG context (συχνά async)
#         rephrased = await get_so_rag_service.rephrase_prompt(
#             project_id=project_id,
#             user_query=request.prompt,
#             session_history=request.history,
#             style_snippet=request.style_snippet,
#         )
        
#         system_prompt = get_so_rag_service.get_system_prompt()
#         # Start streaming in the background
#         background_tasks.add_task(
#             llm_streaming_service.stream_llm_response,
#             prompt=rephrased,
...
@router.post("/project_id/{project_id}/stream", response_class=EventSourceResponse)
async def stream_llm_response_direct(
    project_id: str,
    request: LLMStreamSORequest,
    req: Request,
    ...
    async def event_generator():
        try:
            ...
            rephrased = await so_rag_service.prepare_rephrase_prompt(
                project_id=project_id,
                user_query=request.prompt,
                session_history=request.history
                # style_snippet=request.style_snippet,
            ...
            system_prompt = so_rag_service.get_system_prompt()
