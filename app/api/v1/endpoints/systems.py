...
def list_systems(
    ...
    try:
        ...
        result = service.get_systems_for_project(
            project_id,
            pagination=pagination,  # <-- σωστό
            filters=filters,
            search=search,
        )
...
def get_systems_by_type(
    ...
    try:
        ...
        return create_paginated_response(
            items=systems, total=total_count, skip=skip, limit=limit
        )
    except NotFoundException as e:
        raise HTTPException(status_code=404, detail=str(e))
    except DatabaseException as e:
...
def validate_system_dependencies(
    project_id: str = Path(..., description="The project ID"),
    validation_data: SystemDependencyValidation = Body(
        ..., description="The dependency validation data"
    ),
    db: Session = Depends(get_db),
    service: SystemService = Depends(get_system_service),
):
